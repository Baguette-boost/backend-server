...
async def get_alert_logs(
    ...
    id_list = (await db.execute(stmt_ids)).scalars().all()
...
async def get_unread_alerts_count(current_user: Annotated[Guardian, Depends(get_current_user)], db: AsyncSession = Depends(get_db)):
...
async def mark_alert_as_read(id: int, current_user: Annotated[Guardian, Depends(get_current_user)], db: AsyncSession = Depends(get_db)):
...
async def mark_all_alerts_as_read(current_user: Annotated[Guardian, Depends(get_current_user)], db: AsyncSession = Depends(get_db)):
