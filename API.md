...
[
  {
    ...
    "location": { "address": "역삼로 24", "zoneLabel": "안전구역 이탈", "inSafeZone": false, "isFallConfirmed": true, "lat": 37.501, "lng": 127.036, "updatedAt": "2026-06-19T00:41:00Z" }
  }, {}, ...
]
```
...
안전구역 부분 수정(지정한 필드만 반영). 응답 `200` — 수정된 안전구역(세 필드 모두 포함).
```json
{ "safe_radius": 120 }
```
> 수정할 필드가 하나도 없으면 `400`.
