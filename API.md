...
7. [알림 (Alerts)](#5-알림-alerts)
8. [푸시 알림 (Push)](#6-푸시-알림-push)
9. [대시보드 요약 (Summary)](#7-대시보드-요약-summary)
10. [통화 (Call)](#8-통화-call)
11. [디바이스 데이터 수집 (HW Telemetry)](#9-디바이스-데이터-수집-HW-Telemetry)
12. [Pytorch와 FastAPI 간 내부 API (Internal API)](#10-Pytorch와-FastAPI-간-내부-API-Internal-API)
14. [데이터 모델](#데이터-모델)
...
| 시간 형식 | ISO 8601, UTC (`2026-06-19T00:41:00Z`) — 하드웨어/클라이언트 모두 UTC 로 송수신 |
| 문자 인코딩 | UTF-8 |
...
[
  {
    ...
    "location": { "address": "역삼로 24", "isFallConfirmed": true, "lat": 37.501, "lng": 127.036, "updatedAt": "2026-06-19T00:41:00Z" }
  }, {}, ...
]
```
...
```json
{
  ...
  "createdAt": "2026-06-19T00:41:00Z"
}
```
...
{
  "address": "역삼로 24",
  "latitude": 37.501, "longitude": 127.036, "isFallConfirmed": true, "updatedAt": "2026-06-19T00:41:00Z"
}
```
...
위치 이동 경로(지도 라인).
```json
[ { "latitude": 37.500, "longitude": 127.034, "updatedAt": "2026-06-19T00:30:00Z" }, ... ]
```
...
서버 → 클라이언트 이벤트(JSON):
```json
{ "type": "location",  "personId": "1", "data": { "lat": 37.502, "lng": 127.037, "address": "...", "isFallConfirmed": true, "updatedAt": "..." } }
{ "type": "status",    "personId": "1", "status": "alert" }
{ "type": "alert",     "alert": { "id": "a9", "personId": "1", "type": "wandering", "message": "...", "createdAt": "...", "read": false } }
```
...
{
  "items": [
    { "id": "a1", "personId": "1", "alertType": "wandering", "message": "배회가 감지되었습니다",
      "createdAt": "2026-06-19T00:41:00Z", "read": false }
  ],
  "nextCursor": null
}
```
> `type`: `wandering` · `fall_detected` · `offline`
...
```json
{
  ...
  "latitude": 37.50123,
  "longitude": 127.03645,
  "timestamp": "2026-06-19T00:41:00Z"
}
```
응답 `200`
```json
...
{
  "personId": "1",
  "timestamp": "2026-06-19T00:41:05Z",
  "imuData": {
    "ax": [0.01, 0.05, ...], 
    "ay": [0.98, 1.02, ...],
    "az": [-0.15, -0.10, ...],
...
## 10. Pytorch와 FastAPI 간 내부 API (Internal API)
...
type AlertType = 'wandering' | 'fall_detected' | 'offline';
...
interface TrackedPerson {
  ...
  lastUpdated: string; // ISO 8601 UTC (...Z)
}
