...
```json
{ "pushEnabled": true, "zoneExitAlert": true, "lowBatteryAlert": true }
```
...
## 4. 위치 데이터 (Telemetry)
홈 카드의 배터리 업데이트, 지도 좌표. 실시간성이 핵심 → WebSocket 권장, 폴링 폴백.
...
### `GET /persons/:id/telemetry`
```json
{ "battery": 78, "lastUpdated": "2026-06-19T09:41:00+09:00" }
```
> 오프라인 대상은 `battery`가 `0`이거나 `lastUpdated`가 오래됨 → UI에서 "—" 표기.
...
```
> `type`: `zone_exit` · `low_battery` · `fall_detected` · `offline`
...
### `POST /telemetry/gps`
ESP32 기기가 10초 주기로 위경도 및 배터리 상태 전송
...
ESP32 기기에서 가속도 임계치 초과 시, 직전 5초 ~ 직후 10초(총 15초) 분량의 raw 데이터 전송(POST /predict/fall로 즉시 포워딩)
...
```json
{
  ...
  "fall_detection": { // null 가능
    "is_triggered": true,
    "fall_probability": 0.945
  },
  "wandering_detection": { // null 가능
    "is_triggered": false,
    "wandering_probability": 0.12
  }
}
```
...
type AlertType = 'zone_exit' | 'low_battery' | 'fall_detected' | 'offline';
